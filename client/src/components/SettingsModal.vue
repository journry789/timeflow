<template>
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="$emit('close')">
        <div class="w-full max-w-2xl max-h-[90vh] bg-surface-light dark:bg-surface-dark rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
          <!-- Header -->
          <header class="sticky top-0 bg-surface-light dark:bg-surface-dark border-b border-slate-200 dark:border-slate-700 px-6 pt-5 pb-3 flex items-center justify-between z-10">
            <div>
              <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">设置</h1>
              <p class="text-slate-500 dark:text-slate-400 mt-1 text-xs">管理您的账户偏好与隐私设置</p>
            </div>
            <button
              @click="$emit('close')"
          ...
          <div class="flex-1 overflow-y-auto">
            <form @submit.prevent="handleSubmit" class="p-4 sm:p-6 space-y-4 sm:space-y-6">
              ...
              <section class="rounded-xl bg-slate-50/50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700 overflow-hidden -mx-4 sm:mx-0">
                ...
                <div class="p-4 space-y-5">
                    ...
                    <div class="flex-1 flex items-center gap-4">
                      ...
                      <div class="relative">
                        ...
                        <button
                          ...
                          type="button"
                          @click="removeAvatar"
                          class="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 transition-colors shadow-md"
                          title="删除头像"
                        >
                  ...
                  <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    ...
                    <div class="flex-1 flex items-center gap-3">
                      ...
                      />
                      <p v-if="errors.color" class="text-xs text-red-500 mt-1 absolute top-full">{{ errors.color }}</p>
                    </div>
                  </div>
                </div>
              </section>
          ...
          <!-- Footer Actions -->
          <footer class="sticky bottom-0 bg-surface-light dark:bg-surface-dark border-t border-slate-200 dark:border-slate-700 px-6 py-4 flex justify-end gap-3">
            <button
              type="button"
              @click="handleReset"
  ...
  <AvatarUploadModal
    ...
    @close="showAvatarUploadModal = false"
